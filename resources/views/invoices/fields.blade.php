        ...
        <div class="row gx-10 mb-5">
            ...
            <div class="col-lg-6 col-sm-12">
                <div class="mb-5">
                    ...
                    {{-- {{ Form::select('status', dump($statusArr), isset($invoice) ? $invoice->status : null, ['class' => 'form-select', 'id' => 'status', 'required', 'data-control' => 'select2']) }} --}}
                    <select class="form-select status-selector " id="status" data-control="select2" name="status" >
                        <option @selected(isset($invoice) ? $invoice->status == 1 : null) value="1">{{ __('messages.paid') }}</option>
                        <option @selected(isset($invoice) ? $invoice->status == 0 : null) value="0">{{ __('messages.appointment.pending') }}</option>
                    </select>
                </div>
            </div>
        </div>
            ...
            <table class="table g-5 gs-0 mt-2" id="billTbl">
                <thead>
                    <tr class="border-bottom fs-7 text-gray-700 text-uppercase">
                        ...
                        <th class="{{ getLoggedInUser()->language == 'ar' ? 'text-start' : 'text-end' }}">{{ __('messages.invoice.amount') }}<span class="required"></span></th>
                        <th class="{{ getLoggedInUser()->language == 'ar' ? 'text-start' : 'text-end' }}">{{ __('messages.common.action') }}</th>
                    </tr>
                </thead>
                    ...
                    @if (isset($invoice))
                        ...
                        @foreach ($invoice->invoiceItems as $invoiceItem)
                            <tr class="border-bottom">
                                ...
                                <td class="{{ getLoggedInUser()->language == 'ar' ? 'text-start' : 'text-end' }}">
        ...
        <div class="row justify-content-end ">
            <div class="col-lg-4 col-md-4 col-sm-6 end justify-content-end ">
                <table class="table table-responsive-sm table-row-dashed g-5 gs-0 mb-0  text-gray-700 mr-3">
                    <tbody>
                        <tr
                            class="{{ getLoggedInUser()->language == 'ar' ? 'text-end' : 'text-start' }} text-muted fs-7 text-uppercase gs-0">
                            <td class="font-weight-bold ">{{ __('messages.invoice.sub_total') . ':' }}</td>
                            <td
                                class="font-weight-bold  {{ getLoggedInUser()->language == 'ar' ? 'text-start' : 'text-end' }}">
