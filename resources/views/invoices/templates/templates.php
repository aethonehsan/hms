...
<tr class="border-bottom border-bottom-dashed">
    ...
    <td class="table__item-desc">
        <select class="form-select accountId" name="account_id[]" placeholder="Select Account" id="enquiry-account-id_{{:uniqueId}}" data-id="{{:uniqueId}}" required>
            <option selected="selected" value=0"><?php echo  __('messages.document.select_account')  ?></option>
            {{for accounts}}
    ...
    <td class="<?php echo getCurrentLoginUserLanguageName() == 'ar' ? 'text-start' : 'text-end'  ?>">
