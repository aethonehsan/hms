<div class="row">
    <div class="col-xxl-8">
        <div class="row">
            ...
            <div class="col-md-3 col-6  {{getCurrentLoginUserLanguageName() == 'ar' ? 'text-start' : 'text-end'}}">
            ...
            <div class="col-lg-5  mt-4 {{getCurrentLoginUserLanguageName() == 'ar' ? 'me-lg-auto' : 'ms-lg-auto'}}">
                <div class="border-top">
                    <table class="table table-borderless {{ getLoggedInUser()->thememode ? '' : 'bg-white' }} box-shadow-none mb-0 mt-5">
                        ...
                        <tr>
                            ...
                            <td class="{{ getLoggedInUser()->thememode ? '' : 'bg-white' }} text-gray-900 {{getCurrentLoginUserLanguageName() == 'ar' ? 'text-start' : 'text-end'}} pe-0">{{ getCurrencyFormat($invoice->amount) }}</td>
                        </tr>
                        <tr>
                            <td class="{{ getLoggedInUser()->thememode ? '' : 'bg-white' }} ps-0">{{ __('messages.invoice.discount').(':') }}</td>
                            <td class="{{ getLoggedInUser()->thememode ? '' : 'bg-white' }} text-gray-900 {{getCurrentLoginUserLanguageName() == 'ar' ? 'text-start' : 'text-end'}} pe-0">{{ getCurrencyFormat(($invoice->amount * $invoice->discount / 100)) }}</td>
                        </tr>
                        <tr>
                            <td class="{{ getLoggedInUser()->thememode ? '' : 'bg-white' }} ps-0">{{ __('messages.invoice.total').(':') }}</td>
                            <td class="{{ getLoggedInUser()->thememode ? '' : 'bg-white' }} text-gray-900 {{getCurrentLoginUserLanguageName() == 'ar' ? 'text-start' : 'text-end'}} pe-0">{{ getCurrencyFormat($invoice->amount - ($invoice->amount * $invoice->discount / 100))}}</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
