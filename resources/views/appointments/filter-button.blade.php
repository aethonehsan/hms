    ...
    <div class="d-flex justify-content-end flex-wrap">
        <div class="d-flex align-items-center py-2">
            ...
            <div class="dropdown d-flex align-items-center me-sm-3" wire:ignore>
                <button
                    class="btn btn btn-icon btn-primary text-white dropdown-toggle hide-arrow ps-2 pe-0 {{ getloggedInUser()->language == 'ar' ? 'ms-2' : '' }}"
        ...
        <div class="d-flex align-items-center flex-wrap justify-content-end" wire:ignore>
            <div class="">
                <input class="form-control custom-width px-3 {{ getLoggedInUser()->language == 'ar' ? 'ms-2' : '' }}"
                    placeholder="Pick date range" id="time_range" />
                <b class="caret"></b>
            </div>
            <div class="ms-3 py-3">
                @if (Auth::user()->hasRole('Admin'))
                    <div class="d-flex align-items-center">
                        <a href="{{ route('appointments.create') }}" class="btn btn-primary"
                            data-turbo="false">{{ __('messages.appointment.new_appointment') }}</a>
                    </div>
                @endif
