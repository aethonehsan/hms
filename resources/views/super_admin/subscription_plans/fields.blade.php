<div class="row">
    ...
    <div class="col-md-4 mb-5">
        <div class="form-group">
            ...
            <span class="required"></span>
            {{-- {{ Form::select('frequency', dump($planType), null, ['required', 'id' => 'planType','class' => 'form-select planType','required']) }} --}}
         <select class="form-select status-selector " id="planType" data-control="select2" name="frequency">
            <option value="1">{{ __('messages.employee_payroll.month') }}</option>
            <option value="2">{{ __('messages.employee_payroll.year') }}</option>
        </select>
        </div>
    </div>
...
<div class="d-flex  {{ getLoggedInUser()->language == 'ar' ? 'float-start' : 'float-end'}}">
