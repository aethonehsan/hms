                ...
                <div class="d-sm-flex align-items-center mb-5 mb-xxl-0 text-center text-sm-start">
                    ...
                    <div class="ms-0 ms-sm-10 mt-5 mt-sm-0">
                        ...
                        <span class="text-gray-600 fs-5">
                            <i class="fa-solid fa-envelope {{ getLoggedInUser()->language == 'ar' ? 'ms-1' : 'me-1' }}"></i>
                            <a href="mailto: {{$caseHandler->user->email}}"
                               class="text-gray-600 text-decoration-none fs-5">
                                {{$caseHandler->user->email}}
                            </a>
                        </span>
                        <span class="d-flex align-items-center mb-2 mt-2 {{ getLoggedInUser()->language == 'ar' ? 'ms-5' : 'me-5' }}">
                            @if(!empty($caseHandler->address->address1) || !empty($caseHandler->address->address2) || !empty($caseHandler->address->city) || !empty($caseHandler->address->zip))
                                <i class="fa-solid fa-location-dot text-gray-600 mt-1 {{ getLoggedInUser()->language == 'ar' ? 'ms-3' : 'me-3' }}"></i>
                            @endif
                            <span class="{{ getLoggedInUser()->language == 'ar' ? 'text-end' : 'text-start' }}"> {{ !empty($caseHandler->address->address1) ? $caseHandler->address->address1 : '' }}{{ !empty($caseHandler->address->address2) ? !empty($caseHandler->address->address1) ? ',' : '' : '' }}
...
<div class="mt-7 overflow-hidden">
    <ul class="nav nav-tabs mb-5 pb-1 overflow-auto flex-nowrap text-nowrap" id="myTab" role="tablist">
        <li class="nav-item position-relative  mb-3 {{ getLoggedInUser()->language == 'ar' ? 'ms-7' : 'me-7' }}" role="presentation">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                            ...
                            <span class="fs-5 text-gray-800">{{!empty($caseHandler->user->phone) ? $caseHandler->user->region_code.$caseHandler->user->phone: __('messages.common.n/a')}}</span>
                        </div>
                        <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                            <label for="name"
                                   class="pb-2 fs-5 text-gray-600">{{ __('messages.user.gender').':'  }}</label>
                            <span class="fs-5 text-gray-800">{{ $caseHandler->user->gender == 0 ? __('messages.user.male') : __('messages.user.female')  }}</span>
                        </div>
                        <div class="col-sm-6 d-flex flex-column mb-md-10 mb-5">
                            <label for="name"
