                ...
                <div class="row mt-4 d-flex justify-content-center">
                    <div class="patient-card-modal">
                        <div class="patient-card-header d-flex align-items-center custom-card-header">
                            <div
                                class="flex-1 d-flex align-items-center  {{ getLoggedInUser()->language == 'ar' ? 'ms-3' : 'me-3' }}">
                                <div
                                    class="patient-card-logo {{ getLoggedInUser()->language == 'ar' ? 'ms-4' : 'me-4' }}">
                                    <img src="{{ asset(getSettingValue()['app_logo']['value']) }}" alt="logo"
                        ...
                        <div class="patient-card-body bg-white card-body-radius">
                            <div class="d-flex justify-content-between">
                                ...
                                <div class="w-25">
                                    <div class="{{ getLoggedInUser()->language == 'ar' ? 'text-start' : 'text-end' }}">
                                        <div
                                            class="mb-5 {{ getLoggedInUser()->language == 'ar' ? 'text-start' : 'text-end' }}">
                                            <div
                                                class="qr-code mt-4 {{ getLoggedInUser()->language == 'ar' ? 'me-auto ms-2' : 'ms-auto' }}">
                                            </div>
                                        </div>
                                        <h6 class="text-primary patient-code mb-3 d-none {{ getLoggedInUser()->language == 'ar' ? 'text-start ms-2' : 'text-end ms-2' }}"
                                            id="uniqueCardID">
                                        </h6>
                                        @role('Patient')
                                            <div
                                                class="{{ getLoggedInUser()->language == 'ar' ? 'text-start' : 'text-end' }}">
                                                <a title="{{ __('messages.expense.download') }}"
                                                    class="btn px-5 fs-1 ps-0 download-icon"
                                                    href="{{ url('smart-patient-card-download') . '/' . Auth::user()->patient->id }}"
