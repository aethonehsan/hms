...
<div class="row">
    <div class="form-group col-sm-12 my-0">
        <a href="{{ route('ipd.prescriptions.pdf',$ipdPrescription->id) }}" class="btn btn-sm btn-success {{getCurrentLoginUserLanguageName() == 'ar' ? 'float-start' : 'float-end'}}" target="_blank">
        ...
        <table class="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer w-100 mb-3">
            ...
            @foreach($ipdPrescription->ipdPrescriptionItems as $ipdPrescriptionItem)
                <tr class="fw-bolder text-gray-700 fs-5 {{getCurrentLoginUserLanguageName() == 'ar' ? 'text-start' : 'text-end'}}">
                    ...
                    <td class="{{getCurrentLoginUserLanguageName() == 'ar' ? 'text-end' : 'text-start'}} pt-6">{{ App\Models\Prescription::DOSE_INTERVAL[$ipdPrescriptionItem->dose_interval] ?? 'N\A' }}</td>
                    <td class="{{getCurrentLoginUserLanguageName() == 'ar' ? 'text-end' : 'text-start'}} pt-6">{!! !empty($ipdPrescriptionItem->instruction) ? nl2br(e($ipdPrescriptionItem->instruction)) : __('messages.common.n/a') !!}</td>
                </tr>
            @endforeach
            </tbody>
        </table>
