    ...
    <div class="col-lg-3 col-md-6 col-sm-12">
        <div class="mb-5">
            <div class="mb-5">
                ...
                {{ Form::text('weight', null, ['class' => 'form-control ipdDepartmentFloatNumber', 'data-mask' => '##0,00', 'max' => '200', 'step' => '.01', 'tabindex' => '3', 'placeholder' => __('messages.ipd_patient.weight')]) }}
            </div>
        </div>
    </div>
    <div class="col-lg-3 col-md-6 col-sm-12">
            ...
            <div class="mb-5 d-flex flex-column">
                ...
                <div class="form-check form-switch">
                    <input
                        ...
                        name="is_old_patient" type="checkbox" value="1" id="flexSwitchDefault"
                        {{ $ipdPatientDepartment->is_old_patient ? 'checked' : '' }}>
                </div>
            </div>
        </div>
    </div>
    @if (count($customField) >= 0)
        @foreach ($customField as $field)
            @php
                ...
                $fieldData = isset($ipdPatientDepartment->custom_field[$fieldName])
                    ? $ipdPatientDepartment->custom_field[$fieldName]
                    : null;
            @endphp
            <div class="form-group col-sm-{{ $field['grid'] }} mb-5">
                ...
                @elseif ($field_type == 'toggle')
                    ...
                    <div class="form-check form-switch">
                        <input
                            class="form-check-input w-35px h-20px {{ $field['is_required'] == 1 ? 'dynamic-field' : '' }} "
                            name={{ $fieldName }} type="checkbox" value="1"
                            {{ $fieldData == 0 ? '' : 'checked' }}>
                    </div>
