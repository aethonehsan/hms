                ...
                @foreach($bill['charges'] as $charge)
                    <tr>
                        <td>
                            ...
                            @elseif($charge->charge_type_id == 3)
                                {{ __('messages.charge_filter.others') }}
                            @elseif($charge->charge_type_id == 4)
        ...
        <div class="table-responsive-sm">
            <table class="table table-responsive-sm table-striped box-shadow-none align-middle table-row-dashed fs-6 gx-5 gy-5 dataTable no-footer w-100">
                ...
                @foreach($bill['payments'] as $payment)
                    <tr>
                        <td>
                            @if($payment->payment_mode == 8)
                            PhonePe
                            @elseif($payment->payment_mode == 9)
                            FlutterWave
                            @else
                            {{ __('messages.transaction_filter.'.strtolower($payment->payment_mode_name)) }}
                            @endif
                        </td>
        ...
        <form id="ipdBillForm">
            ...
            <div class="row mb-5">
                <div class="col-lg-12 col-md-12 table-responsive-sm">
                    <table class="table table-responsive-sm table-striped align-middle table-row-dashed fs-6 gx-5 gy-5 dataTable no-footer w-100">
                        <thead class="thead-light">
                        <tr class="text-start {{getCurrentLoginUserLanguageName() == 'ar' ? 'text-end' : 'text-start'}} text-muted fw-bolder fs-7 text-uppercase gs-0">
                        ...
                        <tr>
                            <td>{{ __('messages.lunch_break.bed_charge').':' }}</td>
